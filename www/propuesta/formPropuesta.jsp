...
//System.out.println ("oProp.getCodFacturacion()" + oProp.getCodFacturacion());
...
//                                     parseInt (dateString.substring(0,2)) );
        ...
        if (parseInt (document.getElementById('prop_cod_facturacion').value) > 0 ) {
            ...
            if (premio < minValorCuota ) {
                ...
                if (nuevaCuota < minValorCuota ) {
                    alert ("Modifique la facturación o la cantidad de cuotas porque el valor de la misma es menor a la cuota mínima.\nValor de cuota: $ " + nuevaCuota + ", cuota mínima $ " + minValorCuota );
        ...
        if (document.getElementById ('prop_categoria_persona').value === "J" && 
            document.getElementById ('prop_tom_tipoDoc').value !== "80" ) {
            alert ("Debe informar CUIT para cotizaciones de personas Juridicas");               
            return document.getElementById('prop_tom_tipoDoc').focus();
        }

        if (document.getElementById ('prop_categoria_persona').value === "J" && 
            document.getElementById ('prop_tom_tipoDoc').value === "80"  &&
            ! ( document.getElementById ('prop_tom_nroDoc').value.substr (0,2) === '30' || 
              document.getElementById ('prop_tom_nroDoc').value.substr (0,2) === '33' || 
              document.getElementById ('prop_tom_nroDoc').value.substr (0,2) === '34' ) ) {
            alert ("Para personas Juridicas el cuit tiene que comenzar con 30, 33 o 34");               
            return document.getElementById('prop_tom_nroDoc').focus();
        }
...
//    si es un 6 la tarjeta es Discover   --> resto             
            ...
            alert ("Forma de pago no válida para este producto");
            return false;
        ...
        <td align="center" valign="top" width='100%'>
            <table border='0' width='100%'>
                <form method="post" action="<%= Param.getAplicacion()%>servlet/PropuestaServlet" name='formProp' id='formProp' accept-charset="utf-8">
                    ...
                    <input type="hidden" name="prop_cant_dias"          id="prop_cant_dias"            value="<%= oProp.getcantDias() %>"/>
                    <input type="hidden" name="prop_categoria_persona"  id="prop_categoria_persona"    value="<%= (oProp.getcategoriaPersona() == null ? "" : oProp.getcategoriaPersona()) %>"/>
                <tr>
                    <td>
