 ...
 */
public class GestionDeudaPreliquidacion {
    ...
    public static void main(String[] args) throws Exception{
        ...
        try {
            ...
            cons.setString (2, "COD_TEXTO_GESTION_DEUDA_PRELIQ");
            cons.execute();
            ...
            cons = dbCon.prepareCall(db.getSettingCall("INT_GESTION_DEUDA_PROCESAR ( ? )"));
            cons.registerOutParameter(1, java.sql.Types.DOUBLE);
            cons.setInt (2, 0 ); // no usado
...
System.out.println ("LOTE A PROCESAR: " + lote);
                ...
                dbCon.setAutoCommit(false);
                cons = dbCon.prepareCall(db.getSettingCall("INT_GESTION_DEUDA_ENVIO (?,?,?,?,?,?,?)"));
        ...
        //                }
                        ...
                        if (rs.getString ("EMAIL") == null || rs.getString ("BODY") == null ||
                            ...
                            try {
                                dbCon.setAutoCommit(true);
                                cons2 = dbCon.prepareCall(db.getSettingCall("INT_GESTION_DEUDA_GRABAR (?,?,?,?)"));
                                ...
                                throw new SurException("ERROR EN INT_GESTION_DEUDA_GRABAR: " + se.getMessage());
                           }
                        }
                        rs.close ();
                    }
                ...
                System.out.println ("cantidad de mail enviados: " + cantMail);
                    ...
                    if (bExisteError) {
                        ...
                        dbCon.setAutoCommit(false);
                        cons = dbCon.prepareCall(db.getSettingCall("INT_GESTION_DEUDA_LISTAR (?)"));
                        cons.registerOutParameter(1, java.sql.Types.OTHER);
                        ...
                        rs =  (ResultSet) cons.getObject(1);
                        if (rs != null) {
                            sMensaje.append("RAMA\tPOLIZA\tPRODUCTOR\tMAIL\tRECHAZO\n");
                            while (rs.next()){
                        ...
                        oEmail.setSubject("GESTION DEUDA - RECHAZOS ");
                        oEmail.setContent(sMensaje.toString());

                        LinkedList lDest = oEmail.getDBDestinos(dbCon, 0, "AVISO_GESTION");
