...
<script type="text/javascript">
    var volver   = "<%= volver %>";
    var usuarios = ["JGIL","JCENTENO","VSNEIBRUN","VNERONI","VERONICA", "PINO", "GMERCADAL" ];
...
<%
                    ...
                    tope1              = tope1 + opDet.getimpNeto(); 
                    if (maxAnioMes < opDet.getanioMes()) {
                        maxAnioMes = opDet.getanioMes();
                    }
                    
System.out.println ("loop exterior -------------------------------");
System.out.println ("totalOPDet --> " + totalOPDet);
System.out.println ("maxOPDet --> " + tope1);
...
<%                      for (int ii=0;ii<lCtaCte.size();ii++) { 
                            ...
                            if (oCC.getAnioMes() == opDet.getanioMes() ) {
                                    totalOPDet = totalOPDet + oCC.getImporte();
                                    if (oCC.getOrdene() == 6 || oCC.getOrdene() == 7 ) {
                                        tope1 = tope1 + oCC.getImporte();
                                    }
...
System.out.println ("oCC.getOrdene() -->" + oCC.getOrdene());
System.out.println ("totalOPDet --> " + totalOPDet);
System.out.println ("maxOPDet --> " + tope1);
                        }
...
<%                  if (lCtaCte != null && lCtaCte.size() > 0) {
                        for (int ii=0;ii<lCtaCte.size();ii++) { 
                            CtaCteHis oCC = (CtaCteHis) lCtaCte.get(ii);
                            if (oCC.getAnioMes() > maxAnioMes && oCC.getOrdene() == 9 ) {
                                saldoFinal = oCC.getImporte();
                            }
                        }
                   }
System.out.println ("tope1 --> " + tope1);

System.out.println ("saldo final --> " + saldoFinal);
                    
                 if (oP.getimpOrdenPago() == 0) {
                     if (tope1 < saldoFinal ) {
                        oP.setimpOrdenPago(tope1);
                     } else {
                        oP.setimpOrdenPago(saldoFinal);
                     }
                 }
    ...
    %>
                        <tr>
                            <th colspan="3" align="right">Total Neto de Comisiones Facturadas:</th>
                            <th><%= Dbl.DbltoStrPadRight (oP.getimpNeto() , 2, 9)%></th>
                        </tr>
                        <tr>
                            <th colspan="3" align="right">Tope 1: Comisiones Netas - Serv.Sociales e Ing. Brutos:</th>
                            <th><%= Dbl.DbltoStrPadRight (tope1 , 2, 9)%></th>
                        </tr>
                        <tr>
                            <th colspan="3" align="right">Tope 2: Saldo inicial +/- Mov. Caja Cta.Cte. On Line - OP Pend:</th>
                            <th><%= Dbl.DbltoStrPadRight (saldoFinal , 2, 9)%></th>
                        </tr>
<%              if ( usu.getiCodTipoUsuario() == 0 ) {
    ...
    %>
                        <tr>
                            <th colspan="3" align="right">Total Neto Orden de Pago (menor e/Tope1 y Tope2):</th>
