...
<link rel=stylesheet type="text/css" href="<%=Param.getAplicacion()%>css/estilos.css"/>
   ...
   function PrintCertif ( numCertif, tipo, tcert ) {
    ...
    function CambiarEstado ( num ) {
        ...
        var poliza = prompt('Ingrese número de póliza: ', '');
        if ( Trim(poliza) == '')  {
            alert ("Debe ingresar un número de póliza válido");
            return false;
        } else {
            document.form1.F2num_poliza.value    = poliza;
            document.form1.F2num_propuesta.value = num;
            document.form1.action = "<%= Param.getAplicacion()%>servlet/CaucionServlet";
...
<%                      }
    ...
    %>
                            ...
                            <tr>
                                <td align="left"  class="text">(*)Descripci&oacute;n Tomador:</td>
                                <td align="left"  class="text" colspan='3'><input name="F2nombre" id="F2nombre" size='45' value="<%= sNombre %>"></td>
                            </tr>
                            <tr>
                                <td align="left"  class="text" nowrap>N&uacute;mero Propuesta:</td>
...
<%                                  }  
            ...
            %>
                                      <pg:item>
                                            <tr>
                                                <td align="left" nowrap><%= ( oProp.getTipoPropuesta ().equals ("P") ? "Póliza" : (oProp.getTipoPropuesta().equals ("R") ? "Renovación" : "Endoso")) %></td>
...
<%                                          } else {
                                                    if (oProp.getCodEstado() == 1 && oProp.getCodRama() == 9 && usu.getiCodTipoUsuario() == 0) {
    %>
                                                <td nowrap  align="center">
...
<%                                           if (oProp.getCodRama() != 9 && oProp.getNumCertificado() == 0 &&
                                               (oProp.getCodEstado()== 1 || oProp.getCodEstado()== 2  ||
                                                oProp.getCodEstado()== 3 || oProp.getCodEstado()== 7  || 
